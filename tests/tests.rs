...
mod hooke_spring_tests {
    ...
    fn constructors() {
        ...
        let (pos, vel) = hs.position_velocity();
        ...
        assert_eq!(*pos, 0.0);
        assert_eq!(*vel, 0.0);
        assert_eq!(*hs.target(), 0.0);
        assert_eq!(*hs.damper(), 1.0);
        assert_eq!(*hs.speed(), 1.0);
        ...
        assert_eq!(*vel, 0.0);
        assert_eq!(*hs.target(), 0.0);
        assert_eq!(*hs.damper(), 2.0);
        assert_eq!(*hs.speed(), 3.0);

        let mut hs = HookeSpring::new(Some(2.0), Some(-1.0), Some(3.0), Some(1.5), Some(5.0), Some(ManualClock::wrapped()));
        let (pos, vel) = hs.position_velocity();
        assert_eq!(*pos, 2.0);
        assert_eq!(*vel, -1.0);
        assert_eq!(*hs.target(), 3.0);
        assert_eq!(*hs.damper(), 1.5);
        assert_eq!(*hs.speed(), 5.0);
    }
    #[test]
    fn simulations() {
    ...
    fn edge_cases() {
        ...
        assert!(!&hs.target().is_finite());
        // since damper and speed were not mutated, these should still be 1.0
        assert_eq!(*hs.damper(), 1.0);
        assert_eq!(*hs.speed(), 1.0);
        assert_eq!(hs.elapsed_time(), 0.0); // clock was never advanced
        ...
        let mut hs = HookeSpring::from_damper_speed(1.0, 1.0, Some(ManualClock::wrapped()));
        hs.set_damper_speed(f64::INFINITY, f64::INFINITY);
        ...
        assert!(!&vel.is_finite());
        assert_eq!(*hs.target(), 0.0);
        assert!(!&hs.damper().is_finite());
        assert!(!&hs.speed().is_finite());
        assert_eq!(hs.elapsed_time(), 0.0);
        
        let mut hs = HookeSpring::from_damper_speed(1.0, 1.0, Some(ManualClock::wrapped()));
        hs.set_damper_speed(f64::NAN, f64::NAN);
        let (pos, vel) = hs.position_velocity();
        assert!(!&pos.is_finite());
        assert!(!&vel.is_finite());
        assert_eq!(*hs.target(), 0.0);
        ...
        assert_eq!(*hs.damper(), 1.0);
        assert_eq!(*hs.speed(), 1.0);
        assert!(!hs.elapsed_time().is_finite()); // we messed up the clock with indirection, so this is invalid
        ...
        assert_eq!(*hs.damper(), 1.0);
        assert_eq!(*hs.speed(), 1.0);
        assert!(!hs.elapsed_time().is_finite());
        ...
        assert_eq!(*hs.damper(), 1.0);
        assert_eq!(*hs.speed(), 1.0);
        assert!(!hs.elapsed_time().is_finite()); // we messed up the clock directly, so this is invalid
